...
import { handleGetPayment, handleUpdatePakage } from '../../Store/pakage.slice';
...
const UpdateSubscription: React.FC = () => {
  ...
  const [isLoading, setIsLoading] = useState(true);
    ...
    const fetchPayment = async () => {
        if (subscriptionId) {
          try {
            const { payload } = await dispatch(handleGetPayment({ id: subscriptionId }));
            if (payload.paymentPackage) {
              // Ensure features array exists and has at least one item
              const features = payload.paymentPackage.features?.length > 0 
                ? payload.paymentPackage.features 
                : [''];
    ...
    useEffect(() => {
        dispatch(changeCurrentPath('تعديل الخطة'));
        fetchPayment();
      }, [subscriptionId]);
      ...
      const handleSubmit = async (values: typeof initialValues) => {
        ...
        // Dispatch update action instead of add
        const data = await dispatch(handleUpdatePakage({
          id: subscriptionId,
          ...values
        }));
